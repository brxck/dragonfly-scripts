...
def tag_with_class(element, classes):
    if element == "div"
        element = ""
    Text("%s." % str(element)).execute()
    Text(lib.format.format_dotify(classes)).execute
    Key("space").execute

def tag_with_class(element, tags):
    if element == "div"
        element = ""
    Text("%s#" % str(element)).execute()
    Text(lib.format.format_hashify(tags)).execute
    Key("space").execute
